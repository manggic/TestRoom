...
export function TestCard({ test }: { test: Test }) {
    ...
    <div className="grid grid-cols-1 gap-3 text-gray-800 dark:text-gray-300 text-sm sm:grid-cols-2">
        ...
        <div className="flex items-center gap-2">
            <User size={20} />
            <span className="truncate max-w-[160px] sm:max-w-none" title={createdByName}>
        ...
        {updatedByName && updatedByName !== createdByName && (
            <div className="flex items-center gap-2">
                <User size={20} />
                <span className="truncate max-w-[160px] sm:max-w-none" title={updatedByName}>
    ...
    <div className="flex flex-col sm:flex-row justify-between gap-3 mt-6">
        <div className="flex flex-col sm:flex-row gap-2">
            <Button
                ...
                onClick={() => {
                    if (attempts === 0) {
                        navigate(`/teacher/test/edit/${test.id}?zzzfff=${crypto.randomUUID()}`, {
                            state: { test },
                        });
                    } else {
            ...
            <Button
                ...
                className="cursor-pointer w-full sm:w-auto"
                onClick={() =>
                    navigate(`/teacher/test/preview/${test.id}?zzzfff=${crypto.randomUUID()}`, {
                        state: { test },
                    })
                }
            >
        ...
        {attempts > 0 && (
            <Button
                ...
                variant="outline"
                onClick={() =>
                    navigate(`/teacher/test/attempts/${test.id}?zzzfff=${crypto.randomUUID()}`, {
                        state: { test },
                    })
                }
            >
